...
void Gnss::PathGenerate(){
    ...
    planner.setContourSpiralParameters(path_pattern_,
     ...
     if (path_pattern_ == "contour_spiral" || path_pattern_ == "polygon_boustrophedon") {
...
void Gnss::PathGenerateContourSpiral() {
    ...
    // 保存任务点 (关键拐点)
    task_point = pathMsg;
    task_point.poses.erase(task_point.poses.begin());
    // task_point.poses.erase(task_point.poses.end());

    // Modify the last task point to be the midpoint of the last segment
    // if (task_point.poses.size() >= 2) {
    //     geometry_msgs::Point& last_pt = task_point.poses.back().pose.position;
    //     const geometry_msgs::Point& prev_pt = task_point.poses[task_point.poses.size() - 2].pose.position;
        
    //     last_pt.x = (last_pt.x + prev_pt.x) * 0.5;
    //     last_pt.y = (last_pt.y + prev_pt.y) * 0.5;
    //     if (!pathMsg.poses.empty()) {
    //          pathMsg.poses.back().pose.position = last_pt;
    //     }
...
{
    // 初始化 ROS 节点
    ros::init(argc, argv, "gnss_girdmap");
    ROS_INFO("Starting gnss_girdmap node...");
    ...
    if (!local.setbeginparameter()) {
        ROS_ERROR("Failed to set initial parameters. Exiting...");
        return -1; // 使用非零值表示程序异常退出
    }
    ...
    // 进入 ROS 主循环
    ROS_INFO("Entering ROS spin loop...");
    // 持续发布路径
    // ros::Rate rate(10); 
    // while (ros::ok()) {
    //     local.publishPath(); 
    ...
    // 程序正常退出
    ROS_INFO("Exiting gnss_girdmap node.");
    return 0;
}

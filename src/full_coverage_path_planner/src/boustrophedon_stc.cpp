...
// 计算多边形的轴对齐外接矩形（AABB）
void getBoundingBox(const std::vector<ContourPoint>& polygon,
...
// 检测多边形是否为窄区域（宽度或高度小于工作宽度的指定倍数）
bool isNarrowArea(const std::vector<ContourPoint>& polygon, double w, double threshold = 2.0) {
    double min_x, min_y, max_x, max_y;
    getBoundingBox(polygon, min_x, min_y, max_x, max_y);
    double width  = max_x - min_x;
    double height = max_y - min_y;
    return (width < w * threshold || height < w * threshold);
}

// 生成S弯路径（往复式扫描覆盖外接矩形）
// 根据矩形长宽比自动选择扫描方向（沿长边方向），扫描线间距为 w
std::vector<ContourPoint> generateSBendPath(const std::vector<ContourPoint>& polygon, double w) {
    ...
    if (width >= height) {
        // 水平方向（X）更长或相等：沿 Y 方向步进，每行沿 X 方向来回扫描
        bool left_to_right = true;
        int n_steps = static_cast<int>((height) / w) + 1;
        for (int i = 0; i < n_steps; i++) {
            double y = min_y + i * w;
            if (left_to_right) {
                path.push_back(ContourPoint(min_x, y));
                path.push_back(ContourPoint(max_x, y));
            } else {
                path.push_back(ContourPoint(max_x, y));
                path.push_back(ContourPoint(min_x, y));
            }
        ...
        // 竖直方向（Y）更长：沿 X 方向步进，每列沿 Y 方向来回扫描
        bool bottom_to_top = true;
        int n_steps = static_cast<int>((width) / w) + 1;
        for (int i = 0; i < n_steps; i++) {
            double x = min_x + i * w;
            if (bottom_to_top) {
                path.push_back(ContourPoint(x, min_y));
                path.push_back(ContourPoint(x, max_y));
            } else {
                path.push_back(ContourPoint(x, max_y));
                path.push_back(ContourPoint(x, min_y));
            }
